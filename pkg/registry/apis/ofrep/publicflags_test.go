...
func TestIsPublic(t *testing.T) {
	tests := []struct {
		...
		{name: "empty metadata", metadata: map[string]any{}, want: false},
		{name: "public key absent", metadata: map[string]any{"other": "x"}, want: false},
		...
		{name: `public string "FALSE" (case insensitive)`, metadata: map[string]any{"public": "FALSE"}, want: false},
		{name: "public wrong type (number)", metadata: map[string]any{"public": 1}, want: false},
		{name: "public nil value", metadata: map[string]any{"public": nil}, want: false},
	}
