...
// newSingleEvalBuilder returns an APIBuilder that proxies to a fake upstream
// serving a single-flag OFREP response with the given metadata (so tests control
// whether the flag looks public).
...
// newBulkEvalBuilder returns an APIBuilder that proxies to a fake upstream serving
// a bulk OFREP response with the given flags and status.
...
// newTestBuilder returns an APIBuilder wired to proxy to upstreamURL.
...
// newUnauthReq builds an unauthenticated single-flag-eval request for flagKey,
// with the flagKey and namespace mux vars set for both the root and namespaced
// route handlers.
func newUnauthReq(flagKey string) *http.Request {
	req := httptest.NewRequest(http.MethodPost, "/ofrep/v1/evaluate/flags/"+flagKey, bytes.NewBufferString(`{}`))
	req = mux.SetURLVars(req, map[string]string{"flagKey": flagKey, "namespace": ""})
	requester := &identity.StaticRequester{Type: types.TypeUnauthenticated}
